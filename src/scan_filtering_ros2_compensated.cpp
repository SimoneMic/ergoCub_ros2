    ...
    void scan_callback(const sensor_msgs::msg::LaserScan::ConstPtr& scan_in)
    {
            ...
            & transform_error ))
        {
            ...
            try
            {
                ...
                pcl::fromROSMsg(cloud, *pcl_compensated_cloud);
                ...
                //tf2::doTransform(*pcl_compensated_cloud, *pcl_cloud, compensation_TF);  //maybe unnecessary (only for height fixing and frame name) -> needed afterwards for filtering
            }
            catch(const std::exception& e)
            {
               RCLCPP_ERROR(this->get_logger(), "Cannot Compensate lidar cloud: %s \n", e.what());
               pcl::fromROSMsg(cloud, *pcl_compensated_cloud);  //reset
            }
